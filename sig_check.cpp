...
int main(int argc, char* argv[])
{
    ...
    //l_Ctrl.f_Irq_Mask	= X1TIMER_IRQ_MAIN;
...
//        printf("====================================================\n");
        switch(sig){
            case(10):
                ...
                drv_time_new_s = Event_Ctrl.f_Irq_Sec;
                drv_time_new_u = Event_Ctrl.f_Irq_uSec;
                time_dlt_drv =  (drv_time_new_s  - drv_time_old_s) * 1000000L;
                time_dlt_drv += (drv_time_new_u  - drv_time_old_u);
                cur_event = (int)Event_Ctrl.f_gen_event;
//                printf("%i :USR1\n", n);
...
//                printf("________________________________________________\n");
                ...
                if((cur_event - old_event) != 1){
                    printf("#######%i :USR1 WRONG GEN_EVENT\n", n);
                    printf("   DELAY     %li  local time %li\n", time_dlt, s_time.tv_sec);
                    printf("   DRV_DELAY %li\n", time_dlt_drv);
                    printf("   TIME      %li:%li\n", s_time.tv_sec, s_time.tv_usec);
                    printf("   DRV TIME  %i:%i\n", Event_Ctrl.f_Irq_Sec, Event_Ctrl.f_Irq_uSec);
                    printf("   EVENT INFO GEN_EVENT   %X OLD_EVENT %X DELAY %i\n", cur_event, old_event, Event_Ctrl.f_Irq_Delay);
                    printf("________________________________________________\n");
                }
