...
{   
    ...
    if(x1timer_dev[tmp_slot_num].memmory_base0) {
        ...
        if(tmp_data == ASCII_BOARD_MAGIC_NUM) {
		  ...
		  if(tmp_version_min >= 21){
                        x1timer_dev[tmp_slot_num].master_time = 1;
                        x1timer_dev[tmp_slot_num].X1TIMER_MASTER_SEC   = 148;
                        x1timer_dev[tmp_slot_num].X1TIMER_MASTER_USEC = 144;
                    }
            }
            udelay(10);
    ...
    if(x1timer_dev[tmp_slot_num].memmory_base0){
        ...
        smp_wmb();
        iowrite32(0x0, ((void*)(x1timer_dev[tmp_slot_num].memmory_base0 +X1TIMER_DMA_CTRL)));
        iowrite32(0x0, ((void*)(x1timer_dev[tmp_slot_num].memmory_base0 +X1TIMER_DMA_SIZE)));
        smp_wmb();
    }
    udelay(10);
...

{
    ...
    spin_lock_irqsave(&x1timerdev->irq_lock, flags);
        ...
        if(!(x1timerdev->dev_err_detected)){
            ...
            smp_rmb();
	   iowrite32(0x0, ((void*)(x1timerdev->memmory_base0 +X1TIMER_DMA_CTRL)));
            iowrite32(0x0, ((void*)(x1timerdev->memmory_base0 +X1TIMER_DMA_SIZE)));
            smp_wmb();
            udelay(10);
        }
...
#endif
{
   ...
   mutex_lock(&dev->dev_mut) ;
    ...
    if(!(dev->dev_sts)){
        printk(KERN_ALERT "#####X1TIMER_DO_WORK DEV_STS 0\n");
    ...
    */
	...
	if(dev->master_time){
            tmp_event_l = ioread32 (address + dev->X1TIMER_MASTER_SEC);
            smp_rmb ();
            tmp_event_h = ioread32 (address + dev->X1TIMER_MASTER_USEC);
            smp_rmb ();
            dev->master_sec    = tmp_event_l;
            dev->master_usec  = tmp_event_h;
        }else{
            dev->master_sec    = 0;
            dev->master_usec  = 0;
        }
...
#endif
{
    ...
    if(!dev->dev_sts){
     ...
     if(!dev->memmory_base0) {
         spin_unlock_irqrestore(&dev->irq_lock, flags);  
        printk(KERN_ALERT "##############X1TIMER_INTERRUPT: NO MEMORY \n");
        return IRQ_NONE;
     }
...
*/
	...
	if (_IOC_NR(cmd) < X1TIMER_IOC_MINNR) {
		...
		if (_IOC_TYPE(cmd) == PCIEDOOCS_IOC) {
			if (_IOC_NR(cmd) != _IOC_NR(PCIEDEV_VECTOR_RW)) {
				 mutex_unlock(&dev->dev_mut);
				return -ENOTTY;
			}
		}else{
    ...
    */
    ...
    switch (cmd) {
        ...
        case X1TIMER_EVENT_INFO_EXT:
	       ...
	       event_data_ht.f_master_Sec   = dev->master_sec;
                event_data_ht.f_master_uSec = dev->master_usec;
	...
	if (copy_from_user(&aVectorRW, (device_vector_rw*)arg, sizeof(device_vector_rw))){
		retval = -EFAULT;
		mutex_unlock(&dev->dev_mut);
		return retval;
	}
	...
	for (i = 0; (i < nMaxNumRW) && (!copy_from_user(&aIocRW, pUserRWdata, unSizeOfRW)); ++i,  pUserRWdata += 1)
	{
		if (aIocRW.rw_access_mode<MTCA_SIMPLE_WRITE){
			for ( vi = 0; vi < aIocRW.count_rw;  ++vi){
				tmp_data_for_rw = ioread32(address + aIocRW.offset_rw + vi*4);
				smp_rmb();
				if (copy_to_user((void*)aIocRW.dataPtr + vi*4, &tmp_data_for_rw, 4)) {
					retval = -EFAULT;
					mutex_unlock(&dev->dev_mut);
					return retval;
				}
			...
			for ( vi = 0; vi < aIocRW.count_rw;  ++vi){
				if (copy_from_user(&tmp_data_for_rw, (const void*)aIocRW.dataPtr + vi*4, 4)){
					retval = -EFAULT;
					mutex_unlock(&dev->dev_mut);
					return retval;
				}
	...
	break;
        ...
        case X1TIMER_READ_DMA:
            ...
            timeDMAwait = wait_event_timeout( dev->waitDMA, dev->waitFlag != 0, value );
...
*/
	       ...
	       iowrite32(0x0, ((void*)(dma_reg_address +X1TIMER_DMA_SIZE)));
                smp_wmb();
